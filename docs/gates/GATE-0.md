...
| `pnpm test`                     | `46/46` tests réussis : `32` garde-fous Node, `7` unitaires et `7` dans le runtime Workers                                                    | VERT                                         |
...
| CI distante                     | PR `#5` : `Quality and secrets` vert avec installation figée, `pnpm check`, Gitleaks, audit SCA et smoke HTTP                                 | VERT                                         |
...
| POC Supabase/FedaPay            | aucune mutation, migration métier, Auth/RLS/Queues, sandbox ou charge fournisseur activée                                                     | NON PROUVÉ                                   |
...
| Supabase Auth : Google, OTP e-mail, même UUID, SMTP, intentions et MFA     | NON PROUVÉ | La configuration locale expurgée et le prototype `X-01` existent ; projet distant, fournisseurs, continuation serveur, SMTP, convergence d’identité, MFA et E2E restent non prouvés. ROADMAP, l. 230 ; STI, l. 536–581.                                                    |
...
| Prototype des trois intentions sans autorisation                                             | PARTIEL                   | `X-01` propose trois radios fermées couvertes par des tests structurels, sans présélection, stockage navigateur, rôle ni appel réseau ; la continuation serveur et `X-02..X-06` restent absents. |
...
| Critère                                                                    | Statut                | Preuve locale ou externe attendue                                                                                                            |
| -------------------------------------------------------------------------- | --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| React/Vite PWA Static Assets + API Hono Worker                             | VERT local et externe | Build PWA reproductible, tests runtime Workers, smoke HTTP local et déploiement `loya-s0-topology-poc.morelhouanho.workers.dev` réussis.     |
| Supabase Auth/Postgres/Storage/Queues, CLI local, environnements séparés   | PARTIEL               | `supabase/config.toml` et sonde bornée sont versionnés ; Auth, schéma, RLS, Storage, Queues et environnements distants restent non prouvés.  |
...
| ------------------------------------------------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Prototyper `X-01..X-06`                                      | PARTIEL    | `X-01` est interactif, responsive par CSS et couvert par des tests structurels ; `X-02..X-06`, l’Auth réelle et la suite E2E navigateur versionnée restent absents.                                                       |
...
| Vérifier WCAG 2.2 AA, zoom/reflow, une action principale     | PARTIEL    | `X-01` utilise contrôles natifs, ordre DOM, cibles de 44 px, focus double et styles de reflow ; rapport clavier, lecteur d’écran et audit navigateur restent à produire.                                                  |
